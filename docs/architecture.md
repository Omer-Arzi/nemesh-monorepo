...
### `shir-challenge-month` — recipe relation & homepage carousel

`server/src/api/shir-challenge-month/content-types/shir-challenge-month/schema.json` has an optional `recipe` field: `{ type: "relation", relation: "oneToOne", target: "api::recipe.recipe" }`, unidirectional (no inverse field on `Recipe` — same pattern as `recipe-ingredient`'s `preparationRecipe`). Additive and backward-compatible: existing records keep `recipe: null` with no backfill. Links a challenge month to the recipe made for it; null means "not made/matched yet" (current month) or "back-support gap" (older month), with no way to distinguish the two from the field alone — both fall out of the current month being the only one shown unmatched (see below).

The homepage (`app/(home)/page.tsx`) renders `ShirChallengeCarousel` (`features/home/ShirChallengeCarousel/`) between `FeaturedCategoriesCarousel` and `LatestRecipes` — a horizontal drag-scroll carousel of `ShirChallengeMonthCard`s (`features/home/ShirChallengeMonthCard/`), mechanically identical to `FeaturedCategoriesCarousel` (drag-to-scroll, RTL `scrollLeft` math, edge fades, prev/next arrows) but with its own `SCROLL_AMOUNT`/`FADE_ZONE`/`MAX_CHALLENGE_MONTHS` constants sized for its 240×300px card — parameterized per-instance, not a shared abstraction (two carousels total; a shared hook is a future cleanup if a third appears).

`getShirChallengeCarouselMonths(currentMonthKey)` (`lib/api/services/shirChallengeMonthService.ts`) fetches the full `shir-challenge-month` list (populate `recipe.{title,slug,prepTime,publishedAt,image}`, sorted `monthKey:desc`) and applies the inclusion rule **client-side** in the service function: a month qualifies if its `recipe` is populated and published, or if it is `currentMonthKey`. Non-current unmatched months are dropped. This client-side filter (rather than a server-side `$or`/`$notNull` relation filter) was a deliberate implementation choice — the collection is small and structurally bounded (~12 records/year), so fetching everything is cheap, and it avoids depending on an unverified Strapi filter-operator combination (proving the positive-match case would require writing a relation onto a live content record). An unpublished/draft linked recipe is treated as unmatched (mirrors `mapPreparationRecipe`'s existing published-only guard) so a homepage card never links to a recipe page that 404s for anonymous visitors. The domain `ShirChallengeMonth` type gained a `recipe: ShirChallengeMonthRecipeRef | null` field; `getCurrentChallengeMonth`/`getPreviousChallengeMonth` never populate it, so it is always `null` on those results — no behavior change for the existing status-card feature.

Card states: **matched** (photo, title, `formatHebrewMonthLabel` dateline, `RecipeMeta` with `prepTime` only) links to `ROUTES.RECIPE(slug)`; **current-month placeholder** (unmatched, current month only — a `SoupKitchenIcon`/"בקרוב" image-zone treatment structurally identical to `RecipeCard`'s "no image" fallback, month label in the headline slot, "המתכון בדרך" in the caption slot) links to `ROUTES.TAG(SHIR_CHALLENGE_SLUG)` with an explicit `aria-label`. Zero qualifying months → the section renders `null` (no empty carousel). Full spec: `.claude/handoffs/shir-challenge-carousel/design.md`.
