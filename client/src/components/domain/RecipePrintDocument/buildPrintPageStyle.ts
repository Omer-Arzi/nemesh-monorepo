...
// The ambient (Ctrl+P / File>Print) hide-everything-else logic used to live
// here as a pure-CSS `visibility` rule (`RECIPE_PRINT_AMBIENT_STYLE`). A
// `visibility: hidden` ancestor still occupies its normal layout space, so
// the rest of the live page stayed in flow — just invisible — and the
// browser generated extra printed pages to cover that leftover height
// (pages carrying the repeating `@page` footer but no content). That's now
// handled by `useAmbientRecipePrint`'s `beforeprint`/`afterprint` JS, which
// uses `display: none` instead so hidden elements stop contributing height
// too. See that hook for the full mechanism.
