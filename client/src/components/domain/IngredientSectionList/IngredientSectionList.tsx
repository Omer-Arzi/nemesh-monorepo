 ...
 */
export default function IngredientSectionList({ sections, sx }: Props) {
  if (sections.length === 0) return null;

  // Single unnamed section — skip the title wrapper entirely.
  if (sections.length === 1 && !sections[0].title) {
    return <IngredientList ingredients={sections[0].ingredients} sx={sx} />;
  }
    ...
    <Box sx={sx}>
      {sections.map((section, i) => (
        <Box key={i}>
          ...
          <IngredientList ingredients={section.ingredients} />
        </Box>
      ))}
    </Box>
  );
